...
export default function ServicePage() {
  return <main className="service-page">
    <nav className="service-nav"><strong>EVENT PAGES</strong><div><a href="#works">EXAMPLES</a><a href="#plans">PLANS</a><a href="#flow">FLOW</a><Link href="/admin-demo">ADMIN DEMO</Link></div></nav>
    <section className="service-hero"><p className="service-kicker">FOR WEDDINGS / REUNIONS / ANNIVERSARIES</p><h1>イベントの空気まで<br /><i>伝わる</i>Webページ。</h1><p>招待状を作るのではなく、集まりの時間を楽しみにしてもらうページを。内容整理から出欠管理、公開までまとめてお手伝いします。</p><span className="service-hero-note">二次会・同窓会・周年イベント・記念パーティー</span></section>
    ...
    <section className="service-final"><p className="service-kicker">YOUR NEXT GATHERING</p><h2>次の集まりを、<br />楽しみに変える。</h2><p><Link href="/admin-demo" className="service-demo-link">管理画面のデモを見る →</Link></p></section>
